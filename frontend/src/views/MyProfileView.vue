  ...
  <div class="page-container">
    ...
    <div class="brutal-card accent-yellow" style="margin-top:20px;" v-if="profile">
      ...
      <div v-if="profile.contactInfo" class="profile-contact">
        <i class="pi pi-phone"></i> {{ profile.contactInfo }}
      </div>
    </div>
    ...
    <div class="brutal-grid-2" style="margin-top:20px;">
      <div class="brutal-card accent-cyan quick-card" @click="$router.push('/profile/skills')">
        <div class="quick-icon"><i class="pi pi-bullseye" style="font-size:32px;"></i></div>
      ...
      <div class="brutal-card accent-pink quick-card" @click="$router.push('/profile/hobbies')">
        <div class="quick-icon"><i class="pi pi-palette" style="font-size:32px;"></i></div>
      ...
      <div class="brutal-card accent-green quick-card" @click="$router.push('/profile/gallery')">
        <div class="quick-icon"><i class="pi pi-image" style="font-size:32px;"></i></div>
      ...
      <div class="brutal-card quick-card" @click="$router.push('/profile/password')">
        <div class="quick-icon"><i class="pi pi-lock" style="font-size:32px;"></i></div>
    ...
    <div class="brutal-card accent-blue" style="margin-top:20px;" v-if="posts.length">
      ...
      <div class="post-list">
        <div v-for="post in posts" :key="post.id" class="post-item" @click="router.push(`/community/${post.id}`)">
          ...
          <div class="post-item-meta">
            <button class="card-like-btn" :class="{ liked: post.isLiked, animating: post._animating }" @click.stop="handlePostLike(post)">
              <span class="like-icon-wrap">
                <i class="pi" :class="post.isLiked ? 'pi-star-fill' : 'pi-star'" style="font-size:14px;"></i>
                <span v-if="post._showPlus" class="float-plus">+1</span>
              </span>
              <span class="like-count">{{ formatCount(post.likeCount) }}</span>
            </button>
            <span class="comment-meta"><i class="pi pi-comments"></i> {{ post.commentCount || 0 }}</span>
...
async function handlePostLike(post) {
  ...
  if (!wasLiked) {
    post._animating = true; post._showPlus = true
  ...
  try {
    ...
    else { const res = await togglePostLike(post.id); post.likeCount = res.data?.likeCount ?? post.likeCount }
  } catch {
    post.isLiked = wasLiked; post.likeCount = Math.max(0, (post.likeCount || 0) + (wasLiked ? 1 : -1))
  }
}
...
onMounted(async () => {
  ...
  if (userId) {
    await Promise.all([
      userStore.fetchProfile(userId), userStore.fetchSkills(), userStore.fetchHobbies(), userStore.fetchGallery(),
      getPosts({ authorId: userId, page: 1, size: 10 }).then(res => { posts.value = res.data?.list || [] }),
    ])
  }
})
...
.profile-contact { margin-top: 12px; padding: 8px 12px; display: inline-flex; align-items: center; gap: 6px; background: var(--color-yellow); font-weight: 700; font-size: 13px; border: 2px solid #1A1A1A; }

.quick-card { cursor: pointer; transition: transform 0.2s, box-shadow 0.2s; }
.quick-card:hover { transform: translate(-2px, -2px); }
.quick-icon { margin-bottom: 8px; }
.quick-title { font-size: 18px; font-weight: 900; margin-bottom: 4px; }
.quick-desc { font-size: 13px; color: #888; }

.gallery-preview { display: flex; gap: 8px; margin-top: 8px; }
.gallery-thumb { width: 56px; height: 56px; object-fit: cover; border: 2px solid #1A1A1A; }

.post-list { display: flex; flex-direction: column; gap: 12px; }
.post-item { padding: 14px 16px; border: 2px solid #eee; cursor: pointer; transition: border-color 0.15s, background 0.15s; }
.post-item:hover { border-color: var(--color-blue); background: #f8f9ff; }
.post-item-title { font-size: 16px; font-weight: 800; margin-bottom: 6px; }
.post-item-body { font-size: 13px; color: #666; line-height: 1.6; margin-bottom: 8px; }
.post-item-tags { margin-bottom: 8px; }
.post-item-meta { display: flex; align-items: center; gap: 12px; font-size: 12px; color: #aaa; }
.post-item-time { margin-left: auto; }
.comment-meta { display: flex; align-items: center; gap: 3px; font-weight: 700; color: #888; }

.card-like-btn {
  display: inline-flex; align-items: center; gap: 4px;
  padding: 3px 10px; border: 1.5px solid #ddd; border-radius: 99px;
  background: #fafafa; cursor: pointer; font-family: inherit;
  font-size: 12px; font-weight: 700; color: #888;
  transition: all 0.25s cubic-bezier(0.34, 1.56, 0.64, 1); overflow: visible;
}
.card-like-btn:hover { border-color: #FFD700; background: #FFFBE6; color: #E6A800; transform: translateY(-1px); }
.card-like-btn:active { transform: scale(0.94); }
.card-like-btn.liked { border-color: #1A1A1A; background: linear-gradient(135deg, #F5A623, #FFD700); color: #fff; box-shadow: 2px 2px 0 rgba(0,0,0,0.15), 0 0 12px rgba(245,166,35,0.3); }
...
.like-count { font-weight: 900; min-width: 16px; text-align: center; }
.float-plus { position: absolute; top: -8px; right: -14px; font-size: 11px; font-weight: 900; color: #FFD700; animation: floatUp 0.8s ease-out forwards; }
@keyframes floatUp {
  0% { opacity: 1; transform: translateY(0) scale(1); }
  40% { opacity: 1; transform: translateY(-14px) scale(1.25); }
  100% { opacity: 0; transform: translateY(-24px) scale(0.7); }
}
.card-like-btn.animating .like-icon-wrap { animation: likeBounce 0.5s cubic-bezier(0.34, 1.56, 0.64, 1); }
@keyframes likeBounce {
  0% { transform: scale(1); } 20% { transform: scale(1.5); } 40% { transform: scale(0.75); }
  60% { transform: scale(1.2); } 80% { transform: scale(0.95); } 100% { transform: scale(1); }
}
</style>
